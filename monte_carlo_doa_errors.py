...
def run_simulations(nSources, noisePower, doaError, nSims=200):
    ...
    while simCounter < nSims:
        rca, rcpa, missedDet, sourceRatio = random_simulation(
            nSources=nSources, noisePower=noisePower, doaError=doaError
        )
        if rca == -1:
            continue
        rcas.append(rca)
...
if __name__ == '__main__':
    ...
    for doaError in doaErrors:
        ...
        for noisePower in noisePowers:
            ...
            for nSource in numSources:
                stats = run_simulations(nSources=nSource, noisePower=noisePower, doaError=doaError, nSims=nSims)
    ...
    print("Results saved to 'Results/doaError_results.pkl'")
